...
<html lang="en">
	...
	<body data-spy="scroll" data-target="#navbar"> 
		<div class="wrap">
			...
			<article id="article">
				...
				<!--<script src="http://elecuchi.cafe24.com/s_web/js/nav_pick">//php가안되네</script>-->
				<script>//개발일지네비관련
					var nav = document.getElementsByClassName('nav_pick');
					nav[6].addEventListener('click', cHandler, false);
					function cHandler(){
						var nperi_raw = nav[6].getAttribute('href');
						var nperi = nperi_raw.replace("#","");
						var speri_raw = document.getElementById('<?php echo $_SESSION['period'];?>');
						var speri = speri_raw.getAttribute('id');
						alert ('현재 세션의 피리어드 값은 '+speri);
						alert ('하지만 지금누른 7번째 메뉴의 피리어드 값은 '+nperi);
						if (nperi>speri){ //(nav[6].getAttribute())세션피리어드 값이 애의 아이디값보다 앞서면
							alert('세션보다 지금누른 메뉴의 피리어드가 크다. 아작스로 메뉴의 피리어드를 서버에 보내자');
							$.ajax({
	...
	<script src="http://elecuchi.cafe24.com/s_web/js/bootstrap-scrollspy.js"></script>
	</body>
</html>