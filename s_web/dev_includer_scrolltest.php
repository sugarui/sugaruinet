<?php 	// load형식이므로, 자체적으로 컨텐츠가 완성되어서 불러져야 함
	...
	// DB로부터 컨텐츠 셀렉트. include '../s_web/select.php' 을 변형함
		...
		//데이터 요청및 수신
		$sql =  "
				SELECT p.* FROM su_post_03 AS p LEFT JOIN su_cate_03 AS c ON p.cate = c.cate"." ". 
				$where." "."
	...
	//재료 여부에 따라 더보기 여부를 결정
		...
		if($paravalue){
			$sql = "SELECT id FROM `su_post_03` WHERE $paraname = '$paravalue'"; 
		}else{
			$sql = "SELECT id FROM `su_post_03`";
		}
		$result = mysql_query($sql);
